...
String backendUrl = 'https://www.vpninfotech.com';
...
class _HomePageState extends State<HomePage> {
  ...
  final _border =  Container(width: double.infinity, height: 1.0, color: Colors.red);
  int _radioValue = 0;
            ...
            child:  ListBody(
              children: <Widget>[
                 Row(
                  ...
                  children: <Widget>[
                     const Expanded(child: Text('Initalize transaction from:')),
                     Expanded(
                      child:  Column(
                ...
                Theme(
                  data: Theme.of(context).copyWith(
                    ...
                    textTheme: Theme.of(context).textTheme.copyWith(
                      bodyText2: const TextStyle(color: lightBlue),
                    ),
                  ),
                  child: Builder(
                    builder: (context) {
...
//        addressLine1: '90, Nnebisi Road, Asaba, Deleta State');
  ...
  Future<String?> _fetchAccessCodeFrmServer(String reference) async {
    ...
    String? accessCode;
    try {
      debugPrint("Access code url = $url");
      http.Response response = await http.get(Uri.parse(url));
      accessCode = response.body;
      debugPrint('_fetchAccessCodeFrmServer =>  accessCode : $accessCode');
    } catch (e) {
      setState(() => _inProgress = false);
      debugPrint('_fetchAccessCodeFrmServer =>  ERROR : $e');
      _updateStatus(reference, 'There was a problem getting a  access code form'' the backend: $e');
    }
  ...
  void _verifyOnServer(String? reference) async {
    ...
    try {
      ...
      debugPrint('_verifyOnServer =>  body : $body');
      _updateStatus(reference, body);
    } catch (e) {
      debugPrint('_verifyOnServer =>  ERROR : $e');
      _updateStatus(reference, 'There was a problem verifying %s on the backend: ''$reference $e');
    }
    setState(() => _inProgress = false);
  }
...
class MyLogo extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Image.asset(Assets.splashBackgroundImagePNG,height: 20,width: 20,);
  }
}
